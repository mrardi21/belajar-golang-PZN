...
func TestRouterNamedParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "http://localhost:3001/products/1/items/1", nil)
...
func TestRouterPatternCatchAllParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "http://localhost:3001/images/small/profile.png", nil)
	recorder := httptest.NewRecorder()
